const float Epsilon = 0.001;
const int MaxNumSteps = 512;
const float MaxT = 1024.0;
const float Pi = 3.14159;

float sdfSphere(vec3 P, float R) {
    return length(P) - R;
}

float sdfCube(vec3 P, float S, float r) {
    vec3 Q = abs(P) - vec3(S);
    return length(max(Q, 0.0)) - r;
}

vec2 sdf(vec3 P) {
    vec2 size = vec2(2.0);

    vec2 Q = floor((P.xy + 0.5 * size) / size);
    float m = mod(Q.x + Q.y, 2.0);

    P.z += 2.0 + 2.0 * sin(0.5 * Q.x + iGlobalTime) * sin(0.5 * Q.y + iGlobalTime);
    P.xy = mod(P.xy + 0.5 * size, size) - 0.5 * size;

    float d1 = sdfSphere(P, 1.0);
    float d2 = sdfCube(P, 0.75, 0.25);

    float period = 3.0;
    float a = abs(2.0 * mod(iGlobalTime, period) / period - 1.0);
    float d = mix(d1, d2, a);

    return vec2(0.25 * d, m);
}

vec3 sdfNormal(vec3 P) {
    vec3 h = Epsilon * vec3(1.0, 0.0, 0.0);

    return normalize(vec3(sdf(P + h.xyz).x - sdf(P - h.xyz).x,
                          sdf(P + h.zxy).x - sdf(P - h.zxy).x,
                          sdf(P + h.yzx).x - sdf(P - h.yzx).x));
}

vec2 intersect(vec3 O, vec3 d) {
    float t = 0.0;

    for (int i = 0; i < MaxNumSteps; ++i) {
        vec3 P = O + t * d;
        vec2 result = sdf(P);

        if (result.x < Epsilon)
            return vec2(t, result.y);

        t += result.x;
    }

    return vec2(MaxT, 0.0);
}

vec3 shade(vec3 n, vec3 d, float m) {
    vec3 ambient = vec3(0.25);
    vec3 diffuse = vec3(0.7);
    vec3 specular = vec3(0.5);
    float shininess = 100.0;

    if (m >= 1.0) {
        ambient = vec3(0.01);
        diffuse = vec3(0.15);
        specular = vec3(0.5);
        shininess = 10.0;
    }

    vec3 l = normalize(vec3(1.0, 1.0, 2.0));
    vec3 r = reflect(d, n);

    return ambient
        + diffuse * max(dot(n, l), 0.0)
        + specular * pow(max(dot(r, l), 0.0), shininess);
}

void makeRay(in vec2 fragCoord, out vec3 O, out vec3 d) {
    vec2 xy = 2.0 * fragCoord.xy / iResolution.xy - 1.0;
    xy.y *= iResolution.y / iResolution.x;

    float theta = Pi / 3.0;
    float c = cos(theta);
    float s = sin(theta);
    mat3 R = mat3(1.0, 0.0, 0.0,
                  0.0,   c,   s,
                  0.0,  -s,   c);

    O = R * vec3(0.0, 0.0, 10.0);
    d = R * normalize(vec3(xy, -1.0));
}

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec3 O, d;
    makeRay(fragCoord, O, d);

    vec2 result = intersect(O, d);
    float t = result.x;

    vec3 color = vec3(0.0);

    if (t < MaxT) {
        vec3 P = O + t * d;
        vec3 n = sdfNormal(P);
        color = shade(n, d, result.y);
    }

    vec3 fogColor = vec3(0.125);
    float a = 1.0 - exp(-0.001 * t * t);
    color = mix(color, fogColor, a);

    fragColor = vec4(color, 1.0);
}
